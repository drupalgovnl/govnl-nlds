# DICTU Design System - Oracle APEX Installation Guide

## Overview

This guide explains how to integrate the DICTU Design System with Oracle APEX
Universal Theme.
...
DICTU form components automatically work with APEX items. Just add the wrapper
class:

```html
<div class="dictu-form-field">#LABEL# #ITEM# #ERROR#</div>
```
...
1. Verify installation:
   `SELECT * FROM apex_application_templates WHERE template_name LIKE 'DICTU%'`
...
1. Check package compilation:
   `SELECT * FROM user_errors WHERE name LIKE 'DICTU%'`
...
- **File Integrity**: Use Subresource Integrity (SRI) if loading from external
  sources
- **XSS Protection**: DICTU components use `apex_escape` functions for all user
  input
